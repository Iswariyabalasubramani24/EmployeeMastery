...
if db:
    cursor=db.cursor()
    sql="select userid,username,mailid,mobilenumber,keyskill,emptype from tbl_employee"
    flagid=0
    cursor.execute(sql)
    ...
    for row in results:
        ...
        #print "<a class='btn btn-primary' href='edit_employee.py?q=%s'><i class='icon_plus_alt2'></i></a>"%(mailid)
        print "<a class='btn btn-danger' href='delete_employee.py?q=%s'><i class='icon_close_alt2'></i></a>"%(mailid)
        print "</div></td></tr>"

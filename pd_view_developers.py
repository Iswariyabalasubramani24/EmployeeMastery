...
if(sessionid==""):
    ...
    print "<li><i class='fa fa-th-list'></i>View Token</li></ol></div></div>"
    print "<div class='row'>"
    print "<div class='col-lg-12'>"
    ...
    print "<th>Project Id</th><th>Project Name</th>"
    print "<th>Token No</th><th>Task</th><th>Result</th>"
    print "<th>Task Date</th><th>Completion Date</th></tr>"
    cursor=db.cursor()
    flagid=0
    selectQuery="select projectid,projectname,tokenno,task,result,taskdate,completiondate from tbl_task where empname='%s'"%(sessionid)
    if(cursor.execute(selectQuery)>0):
        ...
        for row in rs:
            ...
            print "<tr><td>%d</td><td>%d</td><td>%s</td><td>%d</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td></tr>"%(int(flagid),int(pid),projectname,int(tokenno),task,result,taskdate,completiondate)
    else:

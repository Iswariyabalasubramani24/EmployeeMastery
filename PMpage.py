...
if sessionid=="":
    ...
    print "<h3 class='page-header'><i class='fa fa-table'></i> PROJECT LEADER</h3>"
    print "<ol class='breadcrumb'>"
    print "<li><i class='fa fa-home'></i><a href='mainpage.py'>Home</a></li>"
    print "<li><i class='fa fa-table'></i>Allocation</li>"
    print "<li><i class='fa fa-th-list'></i>PL Allocation</li></ol></div></div>"
    print "<div class='row'>"
    print "<div class='col-lg-12'>"
    ...
    if db:
        cursor=db.cursor()
        sql="select userid,username,mailid,mobilenumber,keyskill,status,emptype from tbl_employee where emptype='%s'"%("PL")
        flagid=0
        cursor.execute(sql)
        results=cursor.fetchall()
        for row in results:
            ...
            if status=="Allow":
                ...
                print "<td><div class='btn-group'>"
                print "<a class='btn btn-success' href='allowtl.py?q=%s&r=%s'><i class='icon_check_alt2'></i></a>"%(mailid,"Allow")
                print "<a class='btn btn-danger' href='allowtl.py?q=%s&r=%s'><i class='icon_close_alt2'></i></a>"%(mailid,"Deny")
                print "</div></td>"
            else:
                print "<td><div class='btn-group'>"
